...
class LargeForm extends Component {
  ...
  onFormSubmit = e => {
    e.preventDefault();
    const { userName1, userName2, userName3 } = this.state;
    axios.get(`http://localhost:4000/getUser/${userName1},${userName2},${userName3}`, async (err, response, body) => {
    ...
    //     .then(result => {
    //       axios
    //         .get(`https://api.steampowered.com/IPlayerService/GetOwnedGames/v1?key=7D5F2FA02FF09ACA687DE979BE355B30&include_played_free_games=1&format=json&steamid=${
    //           result.steamId
    //         }`)
    //         .then(games => {
    //           console.log(games);
